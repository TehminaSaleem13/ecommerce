...
RSpec.describe ProductsController, type: :controller do
  describe "GET #index" do
    context "when user is not signed in" do
      it "assigns all products to @products" do
        low_stock_product = create(:product, :low_stock)
        out_of_stock_product = create(:product, :out_of_stock)

        get :index

        expect(assigns(:products)).to match_array([low_stock_product, out_of_stock_product])
  ...
  context "when user is not signed in" do
    it "redirects to sign in page" do
      ...
      expect {
        get :new
      }.to raise_error(NoMethodError, /undefined method `products' for nil:NilClass/)
    end
  end
end
...
describe "POST #create" do
  ...
  context "when user is not signed in" do
    it "raises an error due to nil current_user" do
      expect {
...
describe "GET #edit" do
  ...
  context "when owner is signed in" do
    ...
    it "builds image if product has no images" do
      product.product_images.destroy_all
      get :edit, params: { id: product.id }
      
      expect(assigns(:product).product_images.size).to eq(1)
      expect(assigns(:product).product_images.first).to be_new_record
    end
  end
